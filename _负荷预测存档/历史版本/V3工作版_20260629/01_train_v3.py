...
def create_train_features(df):
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            ...
            sh3 = past_same_hour.tail(3)["total_load"].mean() if len(past_same_hour) > 0 else np.nan
            sh7 = past_same_hour.tail(7)["total_load"].mean() if len(past_same_hour) > 0 else np.nan
            sh14 = past_same_hour.tail(14)["total_load"].mean() if len(past_same_hour) > 0 else np.nan
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            ...
            sw4 = past_sw.tail(4)["total_load"].mean() if len(past_sw) > 0 else np.nan
            sw8 = past_sw.tail(8)["total_load"].mean() if len(past_sw) > 0 else np.nan
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            ...
            workday_same_hour.append(wd_past.tail(5)["total_load"].mean() if len(wd_past) > 0 else np.nan)
            restday_same_hour.append(we_past.tail(5)["total_load"].mean() if len(we_past) > 0 else np.nan)
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            ...
            rw3 = past_wh.tail(36)["total_load"].mean() if len(past_wh) > 0 else np.nan
            rw7 = past_wh.tail(84)["total_load"].mean() if len(past_wh) > 0 else np.nan
    ...
    # ===== V3.0 光伏分解 =====
    uid_to_name = dict(zip(user_master["鐢ㄦ埛缂栧彿"], user_master["鐢ㄦ埛鍚嶇О"]))
    df["_user_name"] = df["鐢ㄦ埛缂栧彿"].map(uid_to_name)
    df["pv_capacity"] = df["_user_name"].map(PV_CAPACITY_MAP).fillna(0)
    df["pv_est"] = 0.0
    pv_mask = df["pv_capacity"] > 0
    df.loc[pv_mask, "pv_est"] = (
