...
WEATHER_DIR = BASE_DIR / "1-1负荷预测输入" / "2.预测天气"
...
def load_prediction_weather():
        ...
        for sheet_name in xls.sheet_names:
            try:
                if re.search(r"\d+月\d+日", sheet_name):
                    parsed = _parse_prediction_weather_sheet(fp, sheet_name, xls)
    ...
    weather_df = weather_df.sort_values(["所在市_norm", "datetime"]).drop_duplicates(
...
def _parse_prediction_weather_sheet(fp, sheet_name, xls):
...
def _get_lag_from_history(pred_df, history_df, lag_hours):
    ...
    for _, row in pred_df.iterrows():
        ...
        if len(match) > 0:
            results.append(match.iloc[0]["load"])
            continue
...
def _compute_rolling_from_history(hist_user, pred_g, window_size, stat_func):
    ...
    for _, row in pred_g.iterrows():
        ...
        if len(window) > 0:
            results.append(stat_func(window["load"]))
        else:
            results.append(np.nan)
    return results
...
def _compute_same_hour_from_history(hist_user, pred_g, n_records):
    ...
    for _, row in pred_g.iterrows():
        ...
        val = sh.tail(n_records)["load"].mean() if len(sh) > 0 else np.nan
        results.append(val)
    return results
...
def build_prediction_features(skeleton_df, history_df):
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for _, row in g.iterrows():
            ...
            recent_workhour_3d.append(past_wh_3d["load"].mean() if len(past_wh_3d) > 0 else np.nan)
            recent_workhour_7d.append(past_wh_7d["load"].mean() if len(past_wh_7d) > 0 else np.nan)
