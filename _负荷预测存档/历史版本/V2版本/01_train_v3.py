...
USER_MASTER_PATH = BASE_DIR / "1-1负荷预测输入" / "用户主档案表.xlsx"
WEATHER_DIR = BASE_DIR / "1-1负荷预测输入" / "3.真实天气"
LOAD_DIR = BASE_DIR / "1-1负荷预测输入" / "1.分时段历史用电信息"
...
OUTPUT_LOGS = BASE_DIR / "1-2负荷预测输出" / "logs"
...
def create_train_features(df):
    ...
    for lag in [24, 48, 168]:
        df[f"load_lag_{lag}"] = df.groupby("用户编号")["load"].shift(lag)

    # 滚动统计
    gb = df.groupby("用户编号")["load"]
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            h = g.iloc[i]["datetime"].hour
            past = g.iloc[:i]
            past_same_hour = past[past["datetime"].dt.hour == h]
            sh3 = past_same_hour.tail(3)["load"].mean() if len(past_same_hour) > 0 else np.nan
            sh7 = past_same_hour.tail(7)["load"].mean() if len(past_same_hour) > 0 else np.nan
            sh14 = past_same_hour.tail(14)["load"].mean() if len(past_same_hour) > 0 else np.nan
            same_hour_3d.append(sh3); same_hour_7d.append(sh7); same_hour_14d.append(sh14)
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            h = g.iloc[i]["datetime"].hour
            wd = g.iloc[i]["datetime"].weekday()
            past = g.iloc[:i]
            past_sw = past[(past["datetime"].dt.hour == h) & (past["datetime"].dt.weekday == wd)]
            sw4 = past_sw.tail(4)["load"].mean() if len(past_sw) > 0 else np.nan
            sw8 = past_sw.tail(8)["load"].mean() if len(past_sw) > 0 else np.nan
            same_weekday_hour_4.append(sw4); same_weekday_hour_8.append(sw8)
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            h = g.iloc[i]["datetime"].hour
            past = g.iloc[:i]
            wd_past = past[(past["datetime"].dt.hour == h) & (past["datetime"].dt.weekday < 5)]
            we_past = past[(past["datetime"].dt.hour == h) & (past["datetime"].dt.weekday >= 5)]
            workday_same_hour.append(wd_past.tail(5)["load"].mean() if len(wd_past) > 0 else np.nan)
            restday_same_hour.append(we_past.tail(5)["load"].mean() if len(we_past) > 0 else np.nan)
    ...
    for uid, g in df.groupby("用户编号"):
        ...
        for i in range(len(g)):
            past = g.iloc[:i]
            past_wh = past[(past["datetime"].dt.hour >= 8) & (past["datetime"].dt.hour <= 19)]
            rw3 = past_wh.tail(36)["load"].mean() if len(past_wh) > 0 else np.nan
            rw7 = past_wh.tail(84)["load"].mean() if len(past_wh) > 0 else np.nan
            recent_workhour_3d.append(rw3); recent_workhour_7d.append(rw7)
...
def main():
    ...
    history_for_predict = load_df[["用户编号", "datetime", "load"]].copy()
    history_for_predict["datetime"] = pd.to_datetime(history_for_predict["datetime"])
    history_for_predict = history_for_predict.sort_values(["用户编号", "datetime"])
