...
# 2. 日志
# =========================================================
LOG_FILE = OUTPUT_LOGS / "02_predict_v2_5_weatherfix_log.txt"
...
def build_predict_skeleton(user_master_df):
    history_map_path = OUTPUT_PROCESSED / "user_account_map_v2_5_weatherfix.csv"
...
def merge_predict_weather(predict_df, weather_df):
    ...
    if "wind_speed" not in merged.columns:
        merged["wind_speed"] = np.nan
    if "pressure" not in merged.columns:
        merged["pressure"] = np.nan
    if "visibility" not in merged.columns:
        merged["visibility"] = np.nan
    if "cloud" not in merged.columns:
        merged["cloud"] = np.nan
    if "dew_point" not in merged.columns:
        merged["dew_point"] = np.nan
    if "shortwave_radiation" not in merged.columns:
        merged["shortwave_radiation"] = np.nan
    if "air_quality" not in merged.columns:
        merged["air_quality"] = np.nan

    merged["cooling_degree"] = np.maximum(merged["temperature"] - 24, 0)
    merged["heating_degree"] = np.maximum(18 - merged["temperature"], 0)

    # 光伏交互特征（若脚本版本里需要）
    if "是否有光伏_flag" in merged.columns:
        merged["pv_radiation_effect"] = merged["是否有光伏_flag"] * merged["shortwave_radiation"]
        merged["pv_temp_effect"] = merged["是否有光伏_flag"] * merged["temperature"]
        merged["pv_temp_radiation_effect"] = merged["是否有光伏_flag"] * merged["shortwave_radiation"] * merged["temperature"]
        merged["pv_daytime_radiation"] = merged["是否有光伏_flag"] * merged["shortwave_radiation"] * merged["is_active_hour"]
        merged["pv_workhour_radiation"] = merged["是否有光伏_flag"] * merged["shortwave_radiation"] * merged["is_workhour"]
    ...
    # 打印前20条缺失样本，便于排查
    if "temperature" in merged.columns and merged["temperature"].isna().any():
        miss_sample = merged[merged["temperature"].isna()][
            ["用户编号", "用户名称", "所在市", "所在区", "所在市_norm", "所在区_norm", "datetime"]
        ].head(20)
        log("未匹配到天气的前20条样本：")
        log(miss_sample.to_string(index=False))
...
# =========================================================
def load_models():
    with open(OUTPUT_MODEL / "low_load_classifier_v2_5_weatherfix.pkl", "rb") as f:
        clf_model = pickle.load(f)

    with open(OUTPUT_MODEL / "low_load_regressor_v2_5_weatherfix.pkl", "rb") as f:
        low_reg_model = pickle.load(f)

    with open(OUTPUT_MODEL / "normal_load_regressor_v2_5_weatherfix.pkl", "rb") as f:
        normal_reg_model = pickle.load(f)

    with open(OUTPUT_MODEL / "feature_meta_classifier_v2_5_weatherfix.pkl", "rb") as f:
        clf_meta = pickle.load(f)

    with open(OUTPUT_MODEL / "feature_meta_low_reg_v2_5_weatherfix.pkl", "rb") as f:
        low_reg_meta = pickle.load(f)

    with open(OUTPUT_MODEL / "feature_meta_normal_reg_v2_5_weatherfix.pkl", "rb") as f:
        normal_reg_meta = pickle.load(f)

    return clf_model, low_reg_model, normal_reg_model, clf_meta, low_reg_meta, normal_reg_meta
...
# 12. 历史特征构造
# =========================================================
def build_features_from_history(current_time, row, user_hist):
...
# 13. 递归预测
# =========================================================
def recursive_predict(clf_model, low_reg_model, normal_reg_model,
                      clf_meta, low_reg_meta, normal_reg_meta,
                      history_df, pred_df):
    log("开始递归预测...")
    ...
    pred_df = pred_df.dropna(subset=["datetime", "用户编号"]).copy()
    pred_df = pred_df.sort_values(["用户编号", "datetime"]).reset_index(drop=True)
    pred_df["predicted_load"] = np.nan
    pred_df["low_load_pred_flag"] = np.nan

    log(f"预测输入记录数: {len(pred_df)}")
    log(f"预测用户数: {pred_df['用户编号'].nunique()}")
    log(f"低负荷概率阈值: {LOW_LOAD_PROBA_THRESHOLD}")

    user_hist_map = {}
    for uid, g in hist.groupby("用户编号"):
        if pd.isna(uid):
            continue
        user_hist_map[uid] = g[["datetime", "load"]].copy().sort_values("datetime").reset_index(drop=True)
    ...
    for current_time in all_times:
        ...
        for _, row in step_rows.iterrows():
            ...
            if uid not in user_hist_map:
                user_hist_map[uid] = pd.DataFrame(columns=["datetime", "load"])

            user_hist = user_hist_map[uid].copy()
            row = build_features_from_history(pd.to_datetime(current_time), row, user_hist)
            row["predict_status"] = "待分类"
            feature_rows.append(row)

        step_feature_df = pd.DataFrame(feature_rows)

        cls_mask = step_feature_df["predict_status"] == "待分类"
        if cls_mask.any():
            X_cls = prepare_single_step_features(step_feature_df.loc[cls_mask], clf_meta)

            if hasattr(clf_model, "predict_proba"):
                low_proba = clf_model.predict_proba(X_cls)[:, 1]
                low_flag_pred = (low_proba >= LOW_LOAD_PROBA_THRESHOLD).astype(int)
                step_feature_df.loc[cls_mask, "low_load_pred_proba"] = low_proba
            else:
                low_flag_pred = clf_model.predict(X_cls)
                step_feature_df.loc[cls_mask, "low_load_pred_proba"] = np.nan

            step_feature_df.loc[cls_mask, "low_load_pred_flag"] = low_flag_pred

            # 低负荷模型
            low_mask = cls_mask & (step_feature_df["low_load_pred_flag"] == 1)
            if low_mask.any():
                X_low = prepare_single_step_features(step_feature_df.loc[low_mask], low_reg_meta)
                pred_low = low_reg_model.predict(X_low)
                pred_low = np.where(pred_low < 0, 0, pred_low)
                step_feature_df.loc[low_mask, "predicted_load"] = pred_low
                step_feature_df.loc[low_mask, "predict_status"] = "低负荷模型预测"
                low_count += int(low_mask.sum())

            # 普通模型
            normal_mask = cls_mask & (step_feature_df["low_load_pred_flag"] == 0)
            if normal_mask.any():
                X_normal = prepare_single_step_features(step_feature_df.loc[normal_mask], normal_reg_meta)
                pred_normal = normal_reg_model.predict(X_normal)
                pred_normal = np.where(pred_normal < 0, 0, pred_normal)
                step_feature_df.loc[normal_mask, "predicted_load"] = pred_normal
                step_feature_df.loc[normal_mask, "predict_status"] = "普通模型预测"
        ...
        for _, r in append_rows.iterrows():
            ...
            if uid not in user_hist_map:
                user_hist_map[uid] = add_row.copy()
            else:
                user_hist_map[uid] = pd.concat([user_hist_map[uid], add_row], ignore_index=True)

            user_hist_map[uid] = user_hist_map[uid].drop_duplicates(subset=["datetime"], keep="last")
            user_hist_map[uid] = user_hist_map[uid].sort_values("datetime").reset_index(drop=True)

    final_pred = pd.concat(result_rows, ignore_index=True)
    final_pred = final_pred.sort_values(["用户编号", "datetime"]).reset_index(drop=True)

    log(f"递归预测完成：低负荷模型使用次数={low_count}，普通模型使用次数={normal_count}")
    return final_pred
...
def export_prediction_excels(final_pred):
    ...
    for (uid, uname), g in grouped:
        ...
        out_path = OUTPUT_PREDICTION / f"{safe_name}_v2_5_weatherfix.xlsx"
...
def main():
    ...
    clf_model, low_reg_model, normal_reg_model, clf_meta, low_reg_meta, normal_reg_meta = load_models()

    history_df = pd.read_csv(OUTPUT_PROCESSED / "history_load_for_predict_v2_5_weatherfix.csv", encoding="utf-8-sig")
    ...
    final_pred = recursive_predict(
        ...
        normal_reg_model=normal_reg_model,
        clf_meta=clf_meta,
    ...
    final_pred.to_csv(
        OUTPUT_PREDICTION / "predict_long_v2_5_weatherfix.csv",
        index=False,
        encoding="utf-8-sig"
    )
