...
def parse_args():
    parser = argparse.ArgumentParser(description="负荷预测真实回测 v4.3.6")
...
def build_compare_sheet(validation_df, user_master_df):
    ...
    df["偏差"] = ((df["pred_net_load"] - df["actual_load"]) / df["actual_load"]).replace([np.inf, -np.inf], np.nan).round(4)
    ...
    for type_name, value_col in [("预测", "预测_MWh"), ("实际", "实际_MWh"), ("偏差", "偏差")]:
        ...
        if type_name == "预测":
            ...
            part["日合计(MWh)"] = part["日合计(MWh)"].round(4)
