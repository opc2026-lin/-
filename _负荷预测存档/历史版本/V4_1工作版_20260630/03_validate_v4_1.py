...
def parse_runtime_args():
    parser = argparse.ArgumentParser(description="Load forecast validation runner")
    parser.add_argument("--start-date", help="Validation start date, format: YYYY-MM-DD")
    parser.add_argument("--days", type=int, default=1, help="Number of validation days")
...
def main():
    ...
    if runtime_args.start_date:
        predict_start = pd.Timestamp(runtime_args.start_date).normalize()
        predict_end = predict_start + pd.Timedelta(days=max(int(runtime_args.days or 1), 1))
