...
def save_csv_with_fallback(df, output_path):
    try:
        ...
        log(f"目标文件被占用，已另存为: {fallback_path.name}")
...
def build_prediction_skeleton(user_master_df, weather_pred_df):
    ...
    uid_to_group = dict(zip(user_master_df["用户编号"], user_master_df["user_type_group"]))
    uid_to_property = dict(zip(user_master_df["用户编号"], user_master_df["is_property_user"]))
    uid_to_energy = dict(zip(user_master_df["用户编号"], user_master_df["is_energy_user"]))
    uid_to_industrial = dict(zip(user_master_df["用户编号"], user_master_df["is_industrial_user"]))
...
def main():
    ...
    if guard_override_mask.any():
        ...
        log(f"低负荷保护生效：{guard_override_mask.sum()} 个时段改走普通负荷模型")
    ...
    for h in range(24):
        ...
        for group_name, group_rows in hour_df.groupby("user_type_group"):
            ...
            X_h = encode_prediction_features(group_rows, available_features, meta)
            feature_df.loc[group_rows.index, "pred_total_load"] = normal_regressors[model_key].predict(X_h)
    ...
    result_long = feature_df[[
        ...
        "pv_capacity", "pv_est", "proba_low", "is_low_load_pred", "pred_total_load", "pred_net_load"
    ]].copy()
    result_long_path = save_csv_with_fallback(result_long, OUTPUT_PREDICTION / "prediction_result_v3_long.csv")
    log(f"预测结果长表已保存：{result_long_path.name} | {len(result_long)} 条")
