...
def parse_args():
    parser = argparse.ArgumentParser(description="负荷预测真实回测 v4.3.9")
...
def infer_date_from_filename(file_path: str):
...
def detect_actual_columns(df: pd.DataFrame):
    columns = [str(c).strip() for c in df.columns]
    hour_cols = sorted(
        [col for col in columns if re.fullmatch(r"(1?\d|2[0-4]):00", col)],
        key=lambda x: int(x.split(":")[0]),
    )
    non_hour_cols = [col for col in columns if col not in hour_cols]

    date_col = "电量年月日" if "电量年月日" in columns else None
    name_col = "电力用户名称" if "电力用户名称" in columns else None

    if date_col is None:
        for col in non_hour_cols:
            series = df[col].astype(str).str.strip()
            valid_ratio = series.str.fullmatch(r"\d{8}", na=False).mean()
            if valid_ratio >= 0.5:
                date_col = col
                break
    ...
    return date_col, name_col, hour_cols
...
def load_actual_load(user_master_df, predict_start, predict_end):
    ...
    for fp in files:
        ...
        df = pd.read_excel(fp, sheet_name=0, header=header_row)
        df.columns = [str(col).strip() for col in df.columns]
        date_col, name_col, hour_cols = detect_actual_columns(df)
        if name_col is None:
            continue

        if date_col is not None:
            df[date_col] = df[date_col].astype(str).str.strip()
            valid_mask = df[date_col].str.fullmatch(r"\d{8}", na=False)
            if valid_mask.any():
                df = df[valid_mask].copy()
                df["date"] = pd.to_datetime(df[date_col], format="%Y%m%d", errors="coerce")
            else:
                inferred_date = infer_date_from_filename(fp)
        ...
        df["user_name_norm"] = df[name_col].apply(normalize_text)
        for user_name_norm, user_rows in df.groupby("user_name_norm"):
